...
if ($_POST[ng] == 1) {
    ...
    $rs = mysql_query('SELECT *  FROM group_cat WHERE name = "' . $name . '"');
    if (!mysql_numrows($rs) > 0) {
        $id=(int)$_POST['edit_id'];
    if($id>0){
        mysql_query('UPDATE group_cat SET name= "'.$name.'",descs="'.$desc.'" WHERE group_cat_id='.$id);
        echo '<h1>Успешно обновяване!</h1>';
        }
else{
        mysql_query('INSERT INTO group_cat (name,date_added,descs) VALUES ("' . $name . '",' . time() . ',"' . $desc . '")');
...
$rs = mysql_query('SELECT * FROM group_cat');
echo '<table border="2"><tr><td>Име</td><td>Описание</td><td>Редактирай</td><tr>';
...
if ($_GET ['mode'] == "edit" && $_GET ['id'] > 0) {
    $id = (int) $_GET['id'];
    $rs = mysql_query('SELECT * FROM group_cat WHERE group_cat_id=' . $id);
...
<form action="groups.php" method="post">
    <table border="0">
        <tr>
            <td> Име на група:</td><td><input type="text" name="group_name" value="' . $ed_info ['name'] . '"></td><br/><tr>
